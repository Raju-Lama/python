...
# Group by stock symbol first, then resample for each symbol
ohlc_dict = {
    'rate': ['first', 'max', 'min', 'last'],  # Open, High, Low, Close
    'quantity': 'sum'  # Volume
}

# Set timestamp as index, group by stock symbol, then resample
df.set_index('timestamp', inplace=True)
grouped = df.groupby('stockSymbol')

# Create a list to store OHLC data for each symbol
ohlcv_dfs = []

for name, group in grouped:
    # Resample each group separately
    resampled = group.resample('1T').apply(ohlc_dict)
    # Flatten the MultiIndex
    resampled.columns = ['_'.join(col).strip() for col in resampled.columns.values]
    resampled['stockSymbol'] = name  # Add the stock symbol back
    ohlcv_dfs.append(resampled)

# Combine all the resampled data
ohlcv_df = pd.concat(ohlcv_dfs)

# Plotting with Plotly
fig = go.Figure()

# Add candles for each stock symbol
for symbol in ohlcv_df['stockSymbol'].unique():
    stock_data = ohlcv_df[ohlcv_df['stockSymbol'] == symbol]
    fig.add_trace(go.Candlestick(
        x=stock_data.index,
        open=stock_data['rate_first'],
        high=stock_data['rate_max'],
        low=stock_data['rate_min'],
        close=stock_data['rate_last'],
        name=symbol,
        showlegend=True
    ))
...
fig.update_layout(
    title="OHLCV Candlestick Chart by Stock Symbol",
