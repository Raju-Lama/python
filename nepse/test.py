...
# Paste your JSON data directly here as a Python dictionary
data = {
  "totalCount": 93122,
  "totalPages": 1863,
  "currentPage": 1,
...
# If you want to pretty-print the data to check it
# print(json.dumps(data, indent=4))

# If you want to work with the data, you can manipulate it as required
# For example, accessing the first item
first_item = data['data'][0]
print(f"First item contract number: {first_item['contractNumber']}")
...
for item in data['data']:
    ...
    if symbol not in ohlcv_data:
        ...
        ohlcv_data[symbol]['high'] = max(ohlcv_data[symbol]['high'], rate)
        ohlcv_data[symbol]['low'] = min(ohlcv_data[symbol]['low'], rate)
        ohlcv_data[symbol]['close'] = rate  # the last rate is the closing price
        ohlcv_data[symbol]['volume'] += quantity  # sum up volume

# Prepare for plotting
fig = go.Figure()

# Add candles for each stock symbol
for symbol, ohlcv in ohlcv_data.items():
    fig.add_trace(go.Candlestick(
        x=[item['timestamp'] for item in data['data'] if item['stockSymbol'] == symbol],
        open=[ohlcv['open']] * len([item for item in data['data'] if item['stockSymbol'] == symbol]),
        high=[ohlcv['high']] * len([item for item in data['data'] if item['stockSymbol'] == symbol]),
        low=[ohlcv['low']] * len([item for item in data['data'] if item['stockSymbol'] == symbol]),
        close=[ohlcv['close']] * len([item for item in data['data'] if item['stockSymbol'] == symbol]),
        name=symbol
    ))
